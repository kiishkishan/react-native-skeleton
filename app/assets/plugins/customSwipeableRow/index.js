/*
* Cellcard Selfcare Application 16.6.2019
...
// const maxWidth = Dimensions.get('screen').width * 0.2;
function noop() { }
...
export default class Swipeable extends PureComponent {
  ...
  _handleMoveShouldSetPanResponder = (event, gestureState) => Math.abs(gestureState.dx) > this.props.swipeStartMinDistance;
  ...
  _renderButtons(buttons, isLeftButtons) {
    ...
    let { pan, width } = this.state;
    //--------------
    width = this.props.maxDrabgableWidth
    const canSwipeLeft = this._canSwipeLeft();
    const canSwipeRight = this._canSwipeRight();
  ...
  render() {
    ...
    const transform = [
      {
        translateX: pan.x.interpolate({
          inputRange: [canSwipeLeft ? -width / 4 : 0, canSwipeRight ? width / 4 : 0],
          outputRange: [
